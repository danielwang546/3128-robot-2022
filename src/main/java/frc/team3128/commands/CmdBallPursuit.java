                                            ...
                                            Math.max(
                                                    (VisionConstants.BALL_DECELERATE_START_DISTANCE
                                                            ...
                                                            / (VisionConstants
                                                                    ...
                                                                    - VisionConstants
                                                                            .BALL_DECELERATE_END_DISTANCE),
                                                    0.0),
                                            1.0);

                    m_drivetrain.arcadeDrive(
                            powerMult * VisionConstants.BALL_VISION_kF * multiplier,
                            powerMult * feedbackPower);
    ...
    public boolean isFinished() {
        ...
        */
        if (aimState == BallPursuitState.BLIND) {
            ...
            if (leftVel < VisionConstants.BALL_VEL_THRESHOLD
                    && rightVel < VisionConstants.BALL_VEL_THRESHOLD) {
                plateauCount += 1;
            } else {
                plateauCount = 0;
            }
        ...
        // if in feedback and only 1 ball length away, stop command & give control back to teleop
        // (but
        // not if ball is bouncing and messing up the math)
        if (aimState == BallPursuitState.FEEDBACK) {
            double approxDistance =
                    ballLimelight.calculateDistToGroundTarget(
                            VisionConstants.BALL_TARGET_HEIGHT / 2);
            if (VisionConstants.BALL_DECELERATE_END_DISTANCE > approxDistance
                    && previousVerticalAngle < 20 * Math.PI / 180) {
