...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        m_shooterLimelight =
                new Limelight(
                        "limelight-cog",
                        VisionConstants.TOP_CAMERA_ANGLE,
                        VisionConstants.TOP_CAMERA_HEIGHT,
                        VisionConstants.TOP_FRONT_DIST,
                        0);
        m_ballLimelight =
                new Limelight(
                        "limelight-sog",
                        VisionConstants.BALL_LL_ANGLE,
                        VisionConstants.BALL_LL_HEIGHT,
                        VisionConstants.BALL_LL_FRONT_DIST,
                        0);
        ...
        m_commandScheduler.setDefaultCommand(
                ...
                new CmdArcadeDrive(
                        ...
                        () -> driveHalfSpeed));
        // m_commandScheduler.setDefaultCommand(m_hopper, new CmdHopperDefault(m_hopper,
        // m_shooter::isReady)); //TODO: make input into this good method ???

        autos =
                new AutoPrograms(
                        m_drive, m_shooter, m_intake, m_hopper, m_hood, m_shooterLimelight);
    ...
    private void configureButtonBindings() {
        ...
        // m_rightStick.getButton(1).whenPressed(new SequentialCommandGroup(new
        // CmdRetractHopper(m_hopper), new ParallelCommandGroup(new InstantCommand(() ->
        // m_hood.startPID(12)), new CmdShootRPM(m_shooter, 2700), new CmdHopperShooting(m_hopper,
        // m_shooter::isReady))))
        //                             .whenReleased(new ParallelCommandGroup(new
        // InstantCommand(m_shooter::stopShoot, m_shooter)));
        ...
        //                                     new CmdBallJoystickPursuit(m_drive, m_ballLimelight,
        // m_rightStick::getY, m_rightStick::getTwist, m_rightStick::getThrottle),
        //                                     new CmdExtendIntakeAndRun(m_intake,
        // m_hopper)).beforeStarting(new WaitCommand(0.5)) // Wait 0.5s, then extend intake so as to
        // not
        // block vision
        //                                 );
        ...
        m_rightStick
                ...
                .whenPressed(
                        new SequentialCommandGroup(
                                new CmdRetractHopper(m_hopper),
                                new ParallelCommandGroup(
                                        new InstantCommand(() -> m_hood.startPID(10)),
                                        new CmdShootRPM(m_shooter, 2530),
                                        new CmdHopperShooting(m_hopper, m_shooter::isReady))))
                .whenReleased(
                        new ParallelCommandGroup(
                                new InstantCommand(m_shooter::stopShoot, m_shooter)));
        ...
        m_rightStick
                .getButton(6)
                .whenPressed(new CmdClimbEncoder(m_climber, ClimberConstants.CLIMB_ENC_TO_TOP));
        ...
        m_leftStick
                .getButton(2)
                .whenPressed(new InstantCommand(m_climber::resetLeftEncoder, m_climber));

        m_leftStick.getButton(3).whenPressed(() -> driveHalfSpeed = !driveHalfSpeed);

        // m_leftStick.getButton(5).whenPressed(new CmdClimbEncoder(m_climber,
        // -m_climber.getDesiredTicks(ClimberConstants.SMALL_VERTICAL_DISTANCE)));

        m_leftStick
                .getButton(11)
                .whenPressed(new InstantCommand(m_climber::bothManualExtend, m_climber))
                .whenReleased(new InstantCommand(m_climber::bothStop, m_climber));

        m_leftStick
                .getButton(16)
                .whenPressed(new InstantCommand(m_climber::bothManualRetract, m_climber))
                .whenReleased(new InstantCommand(m_climber::bothStop, m_climber));

        m_leftStick
                .getButton(13)
                .whenPressed(new InstantCommand(m_climber::bothExtend, m_climber))
                .whenReleased(new InstantCommand(m_climber::bothStop, m_climber));

        m_leftStick
                .getButton(14)
                .whenPressed(new InstantCommand(m_climber::bothRetract, m_climber))
                .whenReleased(new InstantCommand(m_climber::bothStop, m_climber));

        m_leftStick
                .getButton(12)
                .whenPressed(new InstantCommand(m_climber::extendPiston, m_climber));
        m_leftStick
                .getButton(15)
                .whenPressed(new InstantCommand(m_climber::retractPiston, m_climber));
        // m_leftStick.getButton(11).whenPressed(new InstantCommand(m_climber::engageBreak,
        // m_climber));
        // m_leftStick.getButton(16).whenPressed(new InstantCommand(m_climber::disengageBreak,
        // m_climber));
    ...
    public void updateDashboard() {
        ...
        NarwhalDashboard.put(
                "range",
                m_shooterLimelight.calculateDistToTopTarget(
                        Constants.VisionConstants.TARGET_HEIGHT));
        ...
        SmartDashboard.putNumber(
                "distance to the limelight",
                m_shooterLimelight.calculateDistToTopTarget(
                        Constants.VisionConstants.TARGET_HEIGHT));
        SmartDashboard.putNumber(
                "ty", m_shooterLimelight.getValue(LimelightKey.VERTICAL_OFFSET, 2));
        SmartDashboard.putNumber(
                "adjusted ty",
                m_shooterLimelight.getValue(LimelightKey.VERTICAL_OFFSET, 5) * (2 / 3));
