...
public class ConstantsInt extends Constants {
    ...
    static {
        ...
        primitiveNumbers =
                Arrays.asList(int.class, long.class, double.class, byte.class, short.class);
    ...
    public static void initTempConstants() {
        for (String cat : categories.keySet()) {
            ...
            for (Field field : categoryClass.getFields()) {
                try {
                    Field backConstantField =
                            categoryClass.getSuperclass().getField(field.getName());
                    field.set(null, backConstantField.get(null));
                    Log.info(
                            "Constants Interface",
                            "Adding Constants Interface Field: " + field.getName());
                    constants.get(cat).add(field);
                } catch (NoSuchFieldException e) {
                    Log.info(
                            "Constants Interface",
                            "Constants Field does not Exist: " + field.getName());
                    ...
                    Log.info(
                            "Constants Interface",
                            "Skipping, Constant set to final: " + field.getName());
                }
            }
        }
    }
