...
/**
 * Wrapper for the WPILib Joystick class. Works with Logitech Extreme 3D Pro and Thrustmaster
 * T16000M.
 */
public class NAR_Joystick {
    ...
    /**
     * POV convention: 0 = up, 45 = top right, 90 = right, 135 = buttom right, 180 = down, 225 =
     * bottom left, 270 = left, 315 = top left We assign indices as angle / 45 [0,7]
     */
    ...
    public NAR_Joystick(int deviceNumber) {
        ...
        for (int i = 1; i < 17; i++) buttons.put(i, new JoystickButton(stick, i));
        for (int i = 0; i < 8; i++) povButtons.put(i, new POVButton(stick, i * 45));
    }
    ...
    /**
     * @return Throttle on [0, 1] where 0 is throttle at bottom, 1 is throttle at top - Default
     *     lower bound is 0.3, upper bound is 0.8, so anything below 0.3 returns 0.3, anything above
     *     0.8 returns 1.
     */
    public double getThrottle() {
        double mappedThrottle = (1 - stick.getThrottle()) / 2;

        if (mappedThrottle < throttleLowerBound) return throttleLowerBound;
        else if (mappedThrottle > throttleUpperBound) return 1;
        else return mappedThrottle;
    }
    ...
    /**
     * @return Raw Joystick throttle on [-1, 1], -1 is throttle all the way up, 1 is throttle all
     *     the way down
     */
