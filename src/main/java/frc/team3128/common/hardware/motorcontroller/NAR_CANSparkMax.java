...
public class NAR_CANSparkMax extends CANSparkMax implements NAR_EMotor {
    ...
    public NAR_CANSparkMax(int deviceNumber, MotorType type) {
        ...
        encoder.setPositionConversionFactor(
                MotorControllerConstants
                        .SPARKMAX_ENCODER_RESOLUTION); // convert rotations to encoder ticks
        encoder.setVelocityConversionFactor(
                MotorControllerConstants.SPARKMAX_RPM_TO_NUpS); // convert rpm to nu/s

        if (RobotBase.isSimulation()) {
            encoderSim =
                    new SimDeviceSim("CANSparkMax[" + this.getDeviceId() + "] - RelativeEncoder");
    ...
    public void follow(NAR_EMotor motor) {
        if (!(motor instanceof CANSparkMax)) {
            throw new RuntimeException(
                    "Bad follow: NAR_CANSparkMax "
                            + getDeviceId()
                            + " attempted to follow non-CANSparkMax motor controller.");
        }
